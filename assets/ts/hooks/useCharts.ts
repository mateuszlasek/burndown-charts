...
export function useCharts(sprintId: string | number) {
    ...
    const [chartLineData, setChartLineData] = useState<(any | null)[]>(endpoints.map((): any | null => null));
    const [errors, setErrors] = useState<(any | null)[]>(endpoints.map((): any | null => null));
